...
# ---------- Build Datasets ----------
def strip_cryptol_comments_with_counts(s: str):
    """
    Remove both // line comments and /* ... */ block comments (supports nesting),
    preserving content inside '...' and "..." string/char literals.

    Returns:
        (clean_text, counts_dict)
        where counts_dict = {"line": <int>, "block": <int>, "total": <int>}
    """
    i, n = 0, len(s)
    out = []
    ...
    # States
    in_line = False       # // ...
    in_block = False      # /* ... */ (nested)
    block_depth = 0
    in_sq = False         # inside '...'
    in_dq = False         # inside "..."
    escape = False        # previous char was backslash in a string?
    ...
    while i < n:
        ...
        # Inside a string/char literal -> copy verbatim, honor escapes
        if in_sq or in_dq:
            out.append(ch)
            if escape:
                escape = False
        ...
        # Inside a // line comment: drop until newline (but keep the newline)
        if in_line:
            if ch == "\n":
                in_line = False
                out.append("\n")
        ...
        # Inside a /* ... */ block comment (support nesting)
        if in_block:
            if ch == "/" and nxt == "*":
                # nested open -> increases depth, also counts as another block comment
                block_depth += 1
                block_count += 1
                i += 2
                continue
    ...
    return "".join(out), {"line": line_count, "block": block_count}
